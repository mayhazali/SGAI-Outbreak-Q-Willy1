...
class Board:
    def __init__(self, dimensions, border, cell_dimensions, pr, h, trainAI, qStats):
        ...
        for s in range(dimensions[0] * dimensions[1]):
            ...
            self.QTable.append([0.1] * 7)

        if not trainAI:
            if h:
                self.loadQTable("QTable_H.txt")
            else:
                self.loadQTable("QTable_NH.txt")

    def saveQTable(self, filename = "QTable.txt_NH"):
        f = open(filename,"a")
    ...
    def loadQTable(self, filename = "QTable.txt"):
        try:
            # Use pre-trained AI
            f = open(filename, "r")
            row = 0
            for line in f:
                line = line.strip()
                lineList = line.split(",")
                lineList = [double(i) for i in lineList]
                self.QTable[row] = lineList
                row+=1
            f.close()
        except:
            print("AI Training Mode - No preloaded Q-Table")
    ...
    def get_possible_moves(self, action, role):
        ...
        if role == "Zombie":
            ...
            for state in self.States:
                if state.person != None:
                    if action == "heal" or action == "vaccinate":
    ...
    def clone(self, L: list):
        NB = Board((self.rows, self.columns), self.display_border, self.display_cell_dimensions, self.Player_Role, self.hasHospital, self.trainAI, self.qStats)
        NB.States = L.copy()
        return NB
    ...
    def heal(self, coords):
        ...
        if p.isZombie:
            ...
            if p.halfCured == False and (p.isInHospital(coords) == False or self.hasHospital == False):
                ...
                p.wasCured = True
                action_type = "full"    
                if (p.isInHospital(coords) == True and self.hasHospital == True):
    ...
    def gameFinished(self):
        DataCollector.humans_remaining = self.num_humans()
        DataCollector.save_ai_data_of_one_game(self.qStats.totalGames)    
        DataCollector.save_stats_data(self.qStats.totalGames) 
        self.gameOver() 